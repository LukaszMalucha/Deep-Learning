...
### Dictionary that maps each line with its id

id2line = {}
for line in lines:
        _line = line.split(' +++$+++ ')
        if len(_line) == 5:
                id2line[_line[0]] = _line[4]               ## maps each line with id

### Creating a list of conversations

conversations_ids = []
for conversation in conversations[:-1]:
        _conversation = conversation.split(' +++$+++ ')[-1][1:-1].replace("'", "").replace(" ", "")  ### split | get last element | remove sqare brackets | remove quotes | remove empty space
...
### Creating the inverse dictionary for answerswords2int
...
def model_inputs():
        inputs = tf.placeholder(tf.int32, [None, None], name = 'input')        ## Q
        targets = tf.placeholder(tf.int32, [None, None], name = 'target')      ## A
